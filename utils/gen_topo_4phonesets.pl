...
($num_sil_states, $num_vowel_states, $num_stop_states, $num_nonstop_states, $sil_phones, $vowel_phones, $stop_phones, $nonstop_phones) = @ARGV;
...
$nonstop_phones =~ s/:/ /g;
$sil_phones =~ s/:/ /g;
$vowel_phones =~ m/^\d[ \d]+$/ || die "$0: bad arguments @ARGV\n";
$stop_phones =~ m/^\d[ \d]*$/ || die "$0: bad arguments @ARGV\n";
$nonstop_phones =~ m/^\d[ \d]*$/ || die "$0: bad arguments @ARGV\n";
...
print "$stop_phones\n";
print "</ForPhones>\n";
for ($state = 0; $state < $num_stop_states; $state++) {
    $statep1 = $state+1;
    print "<State> $state <PdfClass> $state <Transition> $state 0.75 <Transition> $statep1 0.25 </State>\n";
}
print "<State> $num_stop_states </State>\n"; # non-emitting final state.
print "</TopologyEntry>\n";

# nonstop phones
print "<TopologyEntry>\n";
print "<ForPhones>\n";
print "$nonstop_phones\n";
print "</ForPhones>\n";
for ($state = 0; $state < $num_nonstop_states; $state++) {
    $statep1 = $state+1;
    print "<State> $state <PdfClass> $state <Transition> $state 0.75 <Transition> $statep1 0.25 </State>\n";
}
print "<State> $num_nonstop_states </State>\n"; # non-emitting final state.
print "</TopologyEntry>\n";
