...
if $position_dependent_phones; then
  ...
  cat <(set -f; for x in `cat $srcdir/silence_phones.txt`; do for y in "" "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
    <(set -f; for x in `cat $srcdir/vowel_phones.txt`; do for y in "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
    <(set -f; for x in `cat $srcdir/stop_phones.txt`; do for y in "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
    <(set -f; for x in `cat $srcdir/nonstop_phones.txt`; do for y in "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
  ...
  cat $srcdir/silence_phones.txt $srcdir/vowel_phones.txt $srcdir/stop_phones.txt $srcdir/nonstop_phones.txt | \
...
if $share_silence_phones; then
  ...
  cat $srcdir/silence_phones.txt $srcdir/vowel_phones.txt $srcdir/stop_phones.txt $srcdir/nonstop_phones.txt | utils/apply_map.pl $tmpdir/phone_map.txt > $dir/phones/sets.txt
...
cat $srcdir/vowel_phones.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/vowel.txt
cat $srcdir/stop_phones.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/stop.txt
cat $srcdir/nonstop_phones.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/nonstop.txt
...
if $position_dependent_phones; then
  ...
  for suffix in _B _E _I _S; do
    (set -f; for x in `cat $srcdir/vowel_phones.txt`; do echo -n "$x$suffix "; done; echo) >>$dir/phones/extra_questions.txt
  done
  for suffix in _B _E _I _S; do
    (set -f; for x in `cat $srcdir/stop_phones.txt`; do echo -n "$x$suffix "; done; echo) >>$dir/phones/extra_questions.txt
  done
  for suffix in _B _E _I _S; do
    (set -f; for x in `cat $srcdir/nonstop_phones.txt`; do echo -n "$x$suffix "; done; echo) >>$dir/phones/extra_questions.txt
  done
fi
...
if [[ ! -z $phone_symbol_table ]]; then
  ...
  echo "<eps>" | cat - $dir/phones/{silence,vowel,stop,nonstop}.txt | awk -v f=$phone_symbol_table '
  ...
  echo "<eps>" | cat - $dir/phones/{silence,vowel,stop,nonstop,disambig}.txt | \
...
if $position_dependent_phones; then
  cat $dir/phones/{silence,vowel,stop,nonstop}.txt | \
...
for f in silence vowel stop nonstop optional_silence disambig context_indep; do
...
vowellist=`cat $dir/phones/vowel.csl`
stoplist=`cat $dir/phones/stop.csl`
nonstoplist=`cat $dir/phones/nonstop.csl`
